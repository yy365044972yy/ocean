...
//	  System.out.println(getMinDateTimeForToDay());
    ...
    public static List<Date> getStartAndEndTimeByMonth(String selectTime) throws Exception{
    	...
    	if("sevenDay".equals(selectTime) || StringUtils.isEmpty(selectTime)) {
    		Calendar c = Calendar.getInstance();
    		
    		// 近七天
    		createEndTime = getMaxDateTimeForToDay(c.getTime());
    		c.add(Calendar.DAY_OF_YEAR, - 6);  
    		createStartTime = getMinDateTimeForToDay(c.getTime());
    	}else {
    		// 单独某个月份
    		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM");
	 ...
	 * @throws Exception
	 */
	public static Map<String,Date> getStartAndEndTimeByDay(Date dateDay) throws Exception{
		Date startTime = DateUtils.getMinDateTimeForToDay(dateDay);
		Date endTime = DateUtils.getMaxDateTimeForToDay(dateDay);
	...
	/**
	 * 获取昨天的日期字符串，格式yyyy-MM-dd
	 * @return
	 */
	public static Date getYesterday() {
		Date nowtime = new Date();
		nowtime = DateUtils.getMinDateTimeForToDay(nowtime);
		Calendar cal = Calendar.getInstance();
		cal.setTime(nowtime);
		cal.add(Calendar.DAY_OF_YEAR, -1);
		Date date = cal.getTime();
		date = getMinDateTimeForToDay(date);
		return date;
	}
